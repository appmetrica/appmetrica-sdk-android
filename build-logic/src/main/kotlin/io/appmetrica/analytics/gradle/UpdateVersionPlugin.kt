...
private val VERSION_REG = Regex("[0-9]+\\.[0-9]+\\.[0-9]+")
...
class UpdateVersionPlugin : Plugin<Project> {
    ...
    private fun Project.updateGradleConstants(version: String) {
        val filePath = "../appmetrica/appmetrica-sample/constants.gradle"
        val versionCode = getVersionCodeForSample(version).also(::println)
    ...
    private fun Project.updateFile(filePath: String, beforeReg: String, replaceReg: String, afterReg: String, newText: String) {
        ...
        check(file.exists()) { "Not found file $filePath" }

        val regex = Regex("($beforeReg)$replaceReg($afterReg)")
        val text = file.readText()
        val matcher = regex.matchEntire(file.readText())
        checkNotNull(matcher == null) { "Not found regex $regex in file $filePath" }
        file.writeText(text.replace(regex, "\$1$newText\$2"))
    }
