...
import io.appmetrica.analytics.identifiers.internal.AdsIdentifiersProvider
...
class ReflectionAdsIdProviderTest : CommonTest() {

    @get:Rule
    val adsIdentifiersProvider = MockedStaticRule(AdsIdentifiersProvider::class.java)

    private val context = mock<Context>()
    private val parser = mock<ReflectionAdsIdParser>()

    private val providerName = "test"

    private val provider = ReflectionAdsIdProvider(providerName, parser)

    @Test
    fun getData() {
        val resultBundle = mock<Bundle>()
        val result = mock<AdTrackingInfoResult>()
        stubbing(adsIdentifiersProvider.staticMock) {
            on { AdsIdentifiersProvider.requestIdentifiers(same(context), any()) } doReturn resultBundle
        }
    ...
    fun getDataWithRetry() {
        ...
        stubbing(adsIdentifiersProvider.staticMock) {
            on { AdsIdentifiersProvider.requestIdentifiers(same(context), any()) }
    ...
    fun noDataWithRetry() {
        ...
        stubbing(adsIdentifiersProvider.staticMock) {
            on { AdsIdentifiersProvider.requestIdentifiers(same(context), any()) } doThrow RuntimeException(message)
        }
