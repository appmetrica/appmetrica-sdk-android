...
internal class StartupParamsShouldSendStartupForIdentifiersReportAdUrlTest(
    ...
    requestedIdentifiers = listOf(Constants.StartupParamsCallbackKeys.REPORT_AD_URL),
    clientClids = null,
    ...
    companion object {
        ...
        private fun prepareClientIdentifiersHolder(identifierValue: IdentifiersResult?) = when (identifierValue) {
            ...
            REPORT_AD_URL -> prepareEmptyWithReportAdUrl(identifierValue)
            else -> prepareFilledWithReportAdUrl(identifierValue)
        }

        private fun shouldUpdateClids(identifierValue: IdentifiersResult?) =
            identifierValue != null && identifierValue != REPORT_AD_URL

        @Parameterized.Parameters(name = "[{index}] reportAdUrl={0}, isOutdated={1}")
        @JvmStatic
        fun data(): Collection<Array<Any?>> = listOf(
            ...
            // empty with REPORT_AD_URL, not outdated
            arrayOf(
                REPORT_AD_URL,
                false,
                ExpectedStartupResults(
            ...
            // empty with REPORT_AD_URL, outdated
            arrayOf(
                REPORT_AD_URL,
                true,
                ExpectedStartupResults(
    ...
    @Test
    fun `shouldSendStartup for identifiers Report Ad URL`() {
