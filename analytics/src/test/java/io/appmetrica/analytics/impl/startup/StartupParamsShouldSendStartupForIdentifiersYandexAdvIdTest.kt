...
internal class StartupParamsShouldSendStartupForIdentifiersYandexAdvIdTest(
    ...
    requestedIdentifiers = listOf(Constants.StartupParamsCallbackKeys.YANDEX_ADV_ID),
    clientClids = null,
    ...
    companion object {
        ...
        private fun prepareClientIdentifiersHolder(identifierValue: IdentifiersResult?) = when (identifierValue) {
            ...
            YANDEX_ADV_ID -> prepareEmptyWithYandexAdvId(identifierValue)
            else -> prepareFilledWithYandexAdvId(identifierValue)
        }

        private fun shouldUpdateClids(identifierValue: IdentifiersResult?) =
            identifierValue != null && identifierValue != YANDEX_ADV_ID

        @Parameterized.Parameters(name = "[{index}] yandexAdvId={0}, isOutdated={1}")
        @JvmStatic
        fun data(): Collection<Array<Any?>> = listOf(
            ...
            // empty with YANDEX_ADV_ID, not outdated
            arrayOf(
                YANDEX_ADV_ID,
                false,
                ExpectedStartupResults(
    ...
    @Test
    fun `shouldSendStartup for identifiers Yandex Adv ID`() {
