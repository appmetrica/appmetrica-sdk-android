...
class ANRMonitorTest : CommonTest() {
    ...
    private val anrMonitor by setUp { ANRMonitor(defaultListener, null) }

    @Test
    fun `anrTicksCount with null timeout`() {
        val monitor = ANRMonitor(defaultListener, null)
        checkAnrTicksCount(monitor, 5)
    }

    @Test
    fun `anrTicksCount with less default`() {
        val monitor = ANRMonitor(defaultListener, 2)
        checkAnrTicksCount(monitor, 5)
    }

    @Test
    fun `anrTicksCount with custom`() {
        val monitor = ANRMonitor(defaultListener, 10)
    ...
    private fun checkAnrTicksCount(monitor: ANRMonitor, expected: Int) {
        ObjectPropertyAssertions(monitor)
            ...
            .withIgnoredFields("mListeners", "mUiHandler", "mMonitorThread", "mCompleted", "mUiRunnable")
            .checkField("anrTicksCount", expected)
            .checkAll()
    }
}
