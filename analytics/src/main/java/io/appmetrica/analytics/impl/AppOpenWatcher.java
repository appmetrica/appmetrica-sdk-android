...
public class AppOpenWatcher implements ActivityLifecycleManager.Listener {
    ...
    public void setDeeplinkConsumer(@NonNull DeeplinkConsumer deeplinkConsumer) {
        YLogger.info(TAG, "setReporter to %s", deeplinkConsumer);
    ...
    private synchronized void addOrExecuteCommand(@NonNull final NonNullConsumer<DeeplinkConsumer> command) {
        ...
        YLogger.info(TAG, "addOrExecuteCommand. Deeplink consumer: %s", deeplinkConsumerCopy);
    ...
    public void onEvent(@NonNull Activity activity, @NonNull ActivityLifecycleManager.ActivityEvent event) {
        YLogger.info(TAG, "onEvent %s for activity %s", event, activity);
        ...
        if (TextUtils.isEmpty(deeplink) == false) {
            addOrExecuteCommand(new NonNullConsumer<DeeplinkConsumer>() {
                ...
                public void consume(@NonNull DeeplinkConsumer deeplinkConsumer) {
                    YLogger.info(TAG, "Reporting app open: %s", deeplink);
