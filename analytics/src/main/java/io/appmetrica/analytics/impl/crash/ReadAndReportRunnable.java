    ...
    public void run() {
        if (crashFile.exists()) {
            try {
                ...
                if (result != null) {
                    DebugLogger.INSTANCE.info(TAG, "for file %s result is %s", crashFile.getName(), result);
                    crashConsumer.consume(result);
                } else {
                    DebugLogger.INSTANCE.info(TAG, "for file %s result is null", crashFile.getName());
                }
