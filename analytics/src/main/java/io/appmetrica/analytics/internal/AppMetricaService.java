...
public class AppMetricaService extends Service {
    ...
    public void onCreate() {
        super.onCreate();
        DebugLogger.INSTANCE.info(TAG, "AppMetricaService#onCreate()");
        GlobalServiceLocator.init(this.getApplicationContext());
        PublicLogger.init(getApplicationContext());
        DebugLogger.INSTANCE.info(
            TAG,
            "Service was created for owner with package %s, this: %s",
            getPackageName(),
            this
        );
    ...
    public IBinder onBind(Intent intent) {
        DebugLogger.INSTANCE.info(TAG, "Bind to the service with data: %s", intent);
    ...
    public void onRebind(Intent intent) {
        ...
        DebugLogger.INSTANCE.info(TAG, "Rebind to service with data: %s", intent);
        SERVICE_CORE.onRebind(intent);
    }

    @Override
    public void onDestroy() {
        DebugLogger.INSTANCE.info(TAG, "[Service has been destroyed");
    ...
    public boolean onUnbind(Intent intent) {
        DebugLogger.INSTANCE.info(TAG, "Unbind from the service with data: %s", intent);
        ...
        if (action != null && action.startsWith(AppMetricaServiceAction.ACTION_SERVICE_WAKELOCK)) {
            ...
            if (isInvalidIntentData(intent)) {
                DebugLogger.INSTANCE.info(TAG, "Invalid intent data");
                return false;
            }
