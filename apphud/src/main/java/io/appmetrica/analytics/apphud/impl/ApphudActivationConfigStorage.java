...
public class ApphudActivationConfigStorage {

    private static final String TAG = "[ApphudActivationConfigStorage]";

    @NonNull
    private final ModulePreferences modulePreferences;

    public ApphudActivationConfigStorage(
    ...
    @NonNull
    public synchronized ApphudActivationConfig load() {
        ...
        ApphudActivationConfig config = new ApphudActivationConfig(apiKey, deviceId, uuid);
        DebugLogger.INSTANCE.info(TAG, "Loaded config " + config);
        return config;
    }

    public synchronized void save(@NonNull ApphudActivationConfig config) {
