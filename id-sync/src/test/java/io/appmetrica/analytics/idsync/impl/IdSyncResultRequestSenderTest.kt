...
internal class IdSyncResultRequestSenderTest : CommonTest() {
    ...
    @get:Rule
    val networkClientSettingsBuilderRule = constructionRule<NetworkClientSettings.Builder> {
        ...
        on { build() } doReturn networkClientSettings
    }

    private val networkClientSettingsBuilder by networkClientSettingsBuilderRule

    @get:Rule
    val networkClientBuilderRule = constructionRule<NetworkClientBuilder> {
        on { withSettings(any()) } doReturn this.mock
    ...
    fun `sendRequest network client configuration`() {
        ...
        verify(networkClientSettingsBuilder).withSslSocketFactory(sslSocketFactory)
        verify(networkClientSettingsBuilder).withUseCaches(false)
        verify(networkClientSettingsBuilder).withInstanceFollowRedirects(true)
        verify(networkClientSettingsBuilder).withMaxResponseSize(10 * 1024)
