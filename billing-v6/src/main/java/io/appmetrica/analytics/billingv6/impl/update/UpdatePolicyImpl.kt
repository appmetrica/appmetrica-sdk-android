...
class UpdatePolicyImpl(
    ...
    override fun getBillingInfoToUpdate(
        ...
        YLogger.info(TAG, "Products ${productsToUpdate.values.map { it.productId }} should be updated")
    ...
    private fun shouldUpdateBillingInfo(
        ...
        YLogger.debug(TAG, "Product from history $historyEntry now=$now $config")
        ...
        YLogger.debug(TAG, "Found product in storage $storageEntry")
        if (storageEntry.purchaseToken != historyEntry.purchaseToken) {
            YLogger.debug(TAG, "Found product in storage has same purchaseToken")
            return true
        }
