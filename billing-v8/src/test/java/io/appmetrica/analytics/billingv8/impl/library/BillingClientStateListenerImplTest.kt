...
internal class BillingClientStateListenerImplTest : CommonTest() {
    ...
    private val queryPurchaseHistoryParams: QueryPurchasesParams = mock()
    ...
    val queryPurchaseHistoryParamsBuilderRule = MockedConstructionRule(
        QueryPurchasesParams.Builder::class.java
    ) { mock, _ ->
    ...
    fun onBillingSetupFinishedIfOk() {
        ...
        verify(billingClient, times(2)).queryPurchasesAsync(
    ...
    fun onSessionResumedIfBillingClientIsNotReady() {
        ...
        verify(billingLibraryConnectionHolder, never()).addListener(any())
        verify(billingLibraryConnectionHolder, never()).removeListener(any())
        verify(billingClient, never()).queryPurchasesAsync(any<QueryPurchasesParams>(), any())
        verify(updateBillingProgressCallback, times(1)).onUpdateFinished()
    }
}
